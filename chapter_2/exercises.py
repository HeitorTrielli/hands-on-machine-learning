...
train_set, test_set = train_test_split(
    housing, test_size=0.75, random_state=42, stratify=housing.income_cat
)
...
svr_pipeline = Pipeline([("preprocessing", preprocessing), ("svr", svr_model)])
...
final_prediction = final_model.predict(
    housing.drop(columns=["median_house_value", "income_cat"])
)
...
best_param_predict = adapted_rnd_search.predict(
...
svr_knn_pipeline = Pipeline([("preprocessing", preprocessing_knn), ("svr", svr_model)])
...
best_param_predict = grid_search_knn.predict(
    test_set.drop(columns="median_house_value")
)
test_set["prediction"] = best_param_predict
test_set[["median_house_value", "prediction"]].describe()
...
numerical_train_set = train_set_labeless.loc[
    :, train_set_labeless.dtypes == float
].dropna()
...
inverse_transform = pd.DataFrame(
    scc.inverse_transform(scc.fit_transform(numerical_train_set)),
    columns=numerical_train_set.columns,
)

comparison = inverse_transform - numerical_train_set.reset_index(drop=True)
comparison.loc[
    (comparison > 1e-12).any(axis=1)
]  # The differences start to appear around the tolerance value 1e-12.
